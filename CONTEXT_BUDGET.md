...
Files injected via `AGENTS.promptinclude.md` or runtime-native auto-injection:

1. `AGENTS.md` and continuity files (see load order in `AGENTS.md`)

*If prompt context grows too large, defer `INFRASTRUCTURE.md` and runtime files to on-demand load.*
...
| `research` | Uncurated, unverified (if used) | Extract verified facts into `memory/`. |
...
- No subordinate loads `archive/`, `logs/`, or `knowledge/` without explicit instruction.
