...
- Compare root authority with local workspace `AGENTS.md` files.
- If contradictions exist, prefer root authority. Add to `STATE.md` blockers.
...
If `IDENTITY.md` or `SELF.md` is missing, the node is unbound. Follow `BOOTSTRAP.md` if present, otherwise request direction from the Master.
...
- Secrets live in `.a0proj/secrets.env`, `.secrets/`, or environment variables.
